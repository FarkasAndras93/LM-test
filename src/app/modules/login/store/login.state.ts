...
@Injectable()
export class LoginState { // as well need to be added  "implements NgxsOnInit"
    ...
    public loginUser(ctx: StateContext<LoginStateModel>, action: Login): void {
        ...
        return this.controller.loginRequest(action.params).pipe(
            tap((status: boolean) => ctx.dispatch(new LoginCompleted(status, action.params.username))),
            catchError((error) => {
    ...
    public loginUserCompleted(ctx: StateContext<LoginStateModel>, action: LoginCompleted): void {
        ctx.patchState({
